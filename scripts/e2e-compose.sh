...
# Producer Dapr sidecar HTTP API — publishing through it (vs the app's gRPC
# DaprClient) flows through Dapr's TRACED ingress so spans reach Jaeger.
SIDECAR_PUBLISH_URL="${SIDECAR_PUBLISH_URL:-http://localhost:${PRODUCER_DAPR_HTTP_PORT}/v1.0/publish/message-pubsub-kafka/incoming-messages}"
...
# service is registered AND a trace was recorded during THIS run. The traced span
# is generated by publishing through the sidecar's HTTP ingress (the app's gRPC
# DaprClient publish is not traced without app-side OTel instrumentation).
assert_traces_delivering() {
    for _ in 1 2 3 4 5; do
        curl -s -o /dev/null --max-time "$CURL_MAX_TIME" -X POST "$SIDECAR_PUBLISH_URL" \
