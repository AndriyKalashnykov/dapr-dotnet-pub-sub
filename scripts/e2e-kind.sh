...
DAPR_SIDECAR_HTTP_PORT="${DAPR_SIDECAR_HTTP_PORT:-3500}"
...
# The traced span is generated by publishing through the producer Dapr
# sidecar's HTTP ingress (port-forwarded), since the app's gRPC DaprClient
# publish is not traced without app-side OTel. Jaeger's query API is reached
# via a second port-forward. Both use kernel-allocated ephemeral local ports.
assert_traces_delivering() {
    local sidecar_port jaeger_port
    sidecar_port=$(pick_port)
    jaeger_port=$(pick_port)
    "${KUBECTL[@]}" -n "$NS" port-forward deploy/producer "${sidecar_port}:${DAPR_SIDECAR_HTTP_PORT}" >/dev/null 2>&1 &
    PF_PIDS+=($!)
    ...
    for _ in 1 2 3 4 5; do
        curl -s -o /dev/null --max-time "$CURL_MAX_TIME" -X POST \
            "http://localhost:${sidecar_port}/v1.0/publish/message-pubsub-kafka/incoming-messages" \
